...
resource "aws_s3_bucket" "my-statement-bucket" {
...
resource "aws_s3_bucket_versioning" "bucket-versioning" {
  bucket = aws_s3_bucket.my-statement-bucket.id
...
resource "aws_s3_bucket_public_access_block" "my-statement-bucket" {
  bucket = aws_s3_bucket.my-statement-bucket.id
    ...
    Statement = [
      {
        ...
        Action    = "s3:*"
        Resource = [
          "${aws_s3_bucket.my-statement-bucket.arn}/*",
          aws_s3_bucket.my-statement-bucket.arn
        ]
        Condition = {
          Bool = {
    ...
    Statement = [
      {
        Effect   = "Allow"
        Action   = "s3:GetObject"
        Resource = "${aws_s3_bucket.my-statement-bucket.arn}/*"
      }
    ]
  })
}
...
resource "aws_lambda_function" "presigned_url_generator" {
  ...
  environment {
    variables = {
      BUCKET_NAME = aws_s3_bucket.my-statement-bucket.id
